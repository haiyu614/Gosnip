...
import (
	...
	strings2 "Gosnip/src/constant/strings"
...
func errorHandler(channel *amqp.Channel, d amqp.Delivery, requeue bool, logger *logrus.Entry, span *trace.Span) {
	if !requeue { // Nack the message 否定确认
		// 第二个 false 表示不重新放入队列
...
func SummaryConsume(channel *amqp.Channel) {
	...
	for d := range msg {
		...
		if !isAudioFileExist {
			// 音频文件不存在，就从视频文件中提取出来
			// 调用ffmpeg生成音频文件，拼接音频文件名，保存到文件系统
			...
			//* 创建或更新视频
			// 当id字段冲突了，就不创建，而是更新auto_file_name字段
