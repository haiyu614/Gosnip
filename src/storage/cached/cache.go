...
func ScanGet(ctx context.Context, key string, obj interface{}) (bool, error) {
	...
	if x, found := c.Get(key); found {
		// dstVal：指向目标对象，包含目标的类型和值
		dstVal := reflect.ValueOf(obj)
		// dstVal.Elem() 指向目标对象值的指针，使用Set直接将查询结果存到目标对象中
	...
	if wrappedObj.IsDirty() { // todo 这里 isDirty()的意义是？
		logger.WithFields(logrus.Fields{
			"key": key,
...
func getOrCreateCache(name string) *cache.Cache {
	...
	if !ok {
		...
		cc, ok := cacheMaps[name]
		if !ok {
			// 第一个参数（5*time.Minute）：默认过期时间，缓存项在5分钟后过期
			// 第二个参数（10*time.Minute）：清理间隔时间，每10分钟清理一次过期的缓存项
